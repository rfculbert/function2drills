...
const printParams = (paramOne, paramTwo) => `The first parameter is ${paramOne}. The second parameter is ${paramTwo}.` 

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(printParams('hello', "there"))
...
Then, outside of the greeting function, invoke the greeting function, passing in the value firstName and 
lastName of your choice, and a callback function that takes a parameter of fullName. 
...
greeting('Robert', 'Culbert', (fullName) => {
...
const pricesPlusTax = (pricesArray, callBack) => {
    for(let i = 0; i < pricesArray.length; i++){
        totalCost.push(pricesArray[i] + (pricesArray[i] * .20))
    }
    callBack(totalCost)
}

/* 
Invoke the 'pricesPlusTax' function, passing in the 'prices' array and 
a callback function (passing in 'totalCost' as a param) that will print "The new array plus tax = [totalCost]"
*/
...
The inner function should run this logic: 
if the first number passing in is greater than and equal to 5, print the multiplication of the first and second numbers.
 If not, print "Cannot multiply: the first number is smaller than 5." 
*/

// CODE HERE

const multiplyingFactory = (numParam) => {
    return function (num){
        if (numParam >= 5){
            console.log(numParam * num)
        } else {
...
Number 4 here is the second number that will multiply the first number (number 3).
...
Then invoke 'timesFour' again, passing in number 4. Run the code in node, and you should see 20.
*/
